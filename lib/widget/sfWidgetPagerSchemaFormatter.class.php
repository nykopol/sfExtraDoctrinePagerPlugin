...
/**
 * sfWidgetPagerSchemaFormatter base formater
 *
 * @package    sfExtraDoctrinePagerPlugin
 * @author     Desmyter Johan <desmyter.johan@gmail.com>
 */
abstract class sfWidgetPagerSchemaFormatter
{
  protected static
  ...
  protected $formatRowTr		= "<tr>	\n%content%\n</tr>";
  protected $formatThead		= "<thead>	\n%content%\n</thead>";
  protected $formatTfoot		= "<tfoot>	\n%content%\n</tfoot>";
  protected $formatTbody		= "<tbody>	\n%content%\n</tbody>";
  protected $formatTable		= "<table>	\n%thead%\n	%tfoot%\n	%tbody%\n</table>";
  
  protected $formatSortLink	= '<a href="%url%">%anchor%</a>';
  

  /**
   * Constructor
   *
   * @param sfWidgetPagerSchema $widgetSchema
   */
  public function __construct(sfWidgetPagerSchema $widgetSchema)
  {
	...
	public function formatTable($tbody, $thead = '', $tfoot = ''){
		return strtr($this->getFormatTable(), array(
			 '%tbody%' => $tbody,
			 '%thead%' => $thead,
			 '%tfoot%' => $tfoot,
		));
	...
	public function setFormatTable($formatTable) {
		$this->formatTable = $formatTable;
	}

	
	
	public function formatThead($content){
		return strtr($this->getFormatThead(), array('%content%' => $content));
	}
	
	public function getFormatThead() {
		return $this->formatThead;
	}

	public function setFormatThead($formatThead) {
		$this->formatThead = $formatThead;
	}

	
	
	public function formatTfoot($content){
		return strtr($this->getFormatTfoot(), array('%content%' => $content));
	}
	
	public function getFormatTfoot() {
		return $this->formatTfoot;
	}

	public function setFormatTfoot($formatTfoot) {
		$this->formatTfoot = $formatTfoot;
	}

	
	
	
	public function formatTBody($content){
		return strtr($this->getFormatTbody(), array('%content%' => $content));
	}
	
	public function getFormatTbody() {
		return $this->formatTbody;
	}

	public function setFormatTbody($formatTbody) {
		$this->formatTbody = $formatTbody;
	}
	...
	public function getFormatRowTr() {
		return $this->formatRowTr;
	}

	public function setFormatRowTr($formatRowTr) {
		$this->formatRowTr = $formatRowTr;
	}

	
	
	public function formatSortLink($url, $anchor, $sort = '', $sort_type = 'asc', $active = false){
		return strtr($this->getFormatSortLink(), array(
			 '%url%' => $url, 
			 '%anchor%' => $anchor, 
			 '%sort%' => $sort, 
			 '%sort_type%' => $sort_type
	...
	public function getFormatSortLink() {
		return $this->formatSortLink;
	}

	public function setFormatSortLink($formatSortLink) {
		$this->formatSortLink = $formatSortLink;
	}
  ...
  /**
   * Sets a callable which aims to translate Pager labels, errors and help messages
   *
   ...
   *
   * @param sfWidgetPagerSchema $widgetSchema A sfWidgetPagerSchema instance
   */
